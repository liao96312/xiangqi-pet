...
战场 P0 环境模型由用户直接提供于 `C:/Users/Administrator/Desktop/模型`：`P0-01` 指挥台、`P0-02` 长残墙、`P0-03` 短残墙、`P0-04` 红方营帐、`P0-06` 红方军旗、`P0-08` 拒马、`P0-09` 火盆。项目将原始 4K PBR GLB 优化为 `public/assets/environment` 下的 512px 实时版本；缺失的 P0-05 黑营帐与 P0-07 黑军旗由对应红方模型保留几何、UV、木材、绳索和金属后做选择性黑蓝布面换色。原始 4K 文件不进入仓库和安装包。用户已在本任务中明确要求把这些桌面模型用于项目；正式公开 Release 时仍与棋子 HY3 资产一并执行项目所有人的最终再分发确认。

同目录新增的 P1 环境模型中，项目采用 `P1-10` 夯土砖石瓦砾、`P1-11` 碎瓦焦木、`P1-14` 木制角楼、`P1-15` 军阵战鼓、`P1-16` 两轮辎重车和 `P1-17` 长兵器架；全部优化为 512px 运行版本，战鼓从红方源模型生成黑方材质变体。`P1-12` 远山存在漂浮网格与异常投影，`P1-13` 泥地在俯视下产生四角黑面，二者经硬件 WebGL 回读判定不合格，未进入运行目录或安装包。当前环境运行目录共 16 个 GLB、约 28.2MB。
